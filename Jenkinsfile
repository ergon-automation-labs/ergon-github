pipeline {
  ...
  environment {
    ...
    GITHUB_REPO = "ergon-automation-labs/ergon-github"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/github_bot/bin/github_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyGithub.Release.create()'"
          $RELEASE_BIN eval 'BotArmyGithub.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyGithub.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyGithub.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_github/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_github/releases/start_erl.data)
        fi
