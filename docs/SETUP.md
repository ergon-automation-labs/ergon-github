# Getting Started with bot_army_github

This guide walks you through setting up GitHub Bot for local development.
...
DATABASE_NAME=bot_army_github_dev
```
...
GitHub Bot uses the **BotArmy.GenBot** harness to run skills. Skills are discrete, reusable units of work triggered via NATS messages.

### Creating Your First Skill

1. **Start with the template** — `lib/bot_army_github/skills/example.ex` is a fully documented template
2. **Copy and rename** it for your skill:
   ```bash
   cp lib/bot_army_github/skills/example.ex lib/bot_army_github/skills/my_skill.ex
   ```
...
```elixir
defmodule BotArmyGithub.Skills.Classify do
  use BotArmy.Skill

  def name, do: :classify
  def description, do: "Classifies text into categories"
  def nats_triggers, do: ["bot.bot_army_github.command.classify"]
...
```elixir
# In lib/bot_army_github/application.ex or similar

defmodule BotArmyGithub.MyBot do
  use BotArmy.GenBot,
    skills: [
      BotArmyGithub.Skills.Classify,
      BotArmyGithub.Skills.ExtractEntities
    ],
    bot_id: :my_bot
end
```
...
```bash
# Make your changes
edit lib/bot_army_github/skills/my_skill.ex
...
releases: [
  github_bot: [
    applications: [bot_army_github: :permanent]
  ]
]
```

This creates a release named `github_bot` that is deployed to `/opt/ergon/releases/github_bot/` on the server.
...
3. Extracts and deploys to `/opt/ergon/releases/github_bot/`
